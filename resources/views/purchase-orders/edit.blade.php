    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-bold text-2xl text-espresso leading-tight">
                ...
                <span class="text-base font-mono text-charcoal/50 ml-2">{{ $purchaseOrder->po_number }}</span>
            </h2>
        ...
        <div class="bg-white border border-gray-200/60 overflow-hidden shadow-sm rounded-2xl">
            @php
                ...
                if (empty($oldIngredients)) {
                    $oldIngredients = [];
                    foreach ($purchaseOrder->ingredients as $ing) {
                        $oldIngredients[] = [
                            'ingredient_id' => (string) $ing->id,
                            'quantity'      => (string) $ing->pivot->quantity,
                            'unit_price'    => (string) $ing->pivot->unit_price,
                        ];
                    }
                } else {
            ...
            <form method="POST" action="{{ route('purchase-orders.update', $purchaseOrder) }}" class="p-6 sm:p-8 space-y-8"
                  x-data="{
