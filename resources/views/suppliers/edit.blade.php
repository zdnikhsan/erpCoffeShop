<x-app-layout>
    ...
    <div class="py-6">
        <div class="bg-white border border-gray-200/60 overflow-hidden shadow-sm rounded-2xl">
            <form method="POST" action="{{ route('suppliers.update', $supplier) }}" class="p-6 sm:p-8 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <textarea id="address" name="address" rows="3"
                              ...
                              class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm text-charcoal placeholder-charcoal/40 focus:ring-2 focus:ring-latte focus:border-latte transition-colors duration-200 resize-none {{ $errors->has('address') ? 'border-red-400 focus:ring-red-300 focus:border-red-400' : '' }}">{{ old('address', $supplier->address) }}</textarea>
                    @error('address')
                        <p class="mt-1.5 text-xs text-red-500 font-medium">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" id="payment_terms" name="payment_terms" value="{{ old('payment_terms', $supplier->payment_terms) }}" min="0"
                           placeholder="0"
