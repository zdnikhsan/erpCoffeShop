<x-app-layout>
    ...
    <div class="py-6 space-y-6">
        ...
        <div class="bg-white border border-gray-200/60 rounded-2xl shadow-sm p-4">
            <form method="GET" action="{{ route('ingredients.index') }}" class="flex items-center gap-3">
                <div class="relative flex-1">
                    ...
                    <input type="text" name="search" value="{{ $search }}" placeholder="Cari nama atau kode SKU bahan baku..."
                ...
                <table class="w-full text-sm text-left">
                    <thead>
                        <tr class="bg-espresso/5 border-b border-gray-200/60">
                            ...
                            <th class="px-6 py-4 font-semibold text-espresso text-xs uppercase tracking-wider">Stok</th>
                            <th class="px-6 py-4 font-semibold text-espresso text-xs uppercase tracking-wider">Satuan</th>
                            <th class="px-6 py-4 font-semibold text-espresso text-xs uppercase tracking-wider">Safety Stock</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse ($ingredients as $ingredient)
                            <tr class="hover:bg-latte/5 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4 font-semibold text-charcoal">{{ $ingredient->name }}</td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold {{ $ingredient->isLowStock() ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700' }}">
                                ...
                                <td class="px-6 py-4 text-charcoal/80">{{ $ingredient->unit }}</td>
                                <td class="px-6 py-4 text-charcoal/70">{{ number_format($ingredient->safety_stock, 2) }}</td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center gap-1">
                                        ...
                                        {{-- Edit & Hapus — hanya owner/manager --}}
                                        @hasanyrole('owner|manager')
                                        ...
                                        <form method="POST" action="{{ route('ingredients.destroy', $ingredient) }}"
                                            ...
                                            @method('DELETE')
                                            <button type="button"
                                                    @click="if(confirm('Yakin ingin menghapus bahan baku \'{{ $ingredient->name }}\'?')) $refs.deleteForm.submit()"
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-charcoal/50 font-medium">Belum ada data bahan baku.</p>
                                        @hasanyrole('owner|manager')
                                        <a href="{{ route('ingredients.create') }}" class="mt-2 text-sm text-latte-dark hover:text-espresso font-semibold transition-colors duration-200">
