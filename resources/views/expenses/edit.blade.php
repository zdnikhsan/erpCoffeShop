<x-app-layout>
    ...
    <div class="py-6">
        <div class="bg-white border border-gray-200/60 overflow-hidden shadow-sm rounded-2xl">
            <form method="POST" action="{{ route('expenses.update', $expense) }}" class="p-6 sm:p-8 space-y-6">
                @csrf
                @method('PUT')

                {{-- Info Nomor --}}
                <div class="flex items-center gap-3 p-4 bg-espresso/5 rounded-xl">
                    <svg class="w-5 h-5 text-espresso shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <span class="text-sm font-medium text-espresso">Nomor: <strong class="font-mono">{{ $expense->expense_number }}</strong></span>
                </div>
                ...
                <div>
                    ...
                    <input type="number" id="amount" name="amount" value="{{ old('amount', $expense->amount) }}" min="1" step="1"
                ...
                <div>
                    ...
                    <input type="date" id="date" name="date" value="{{ old('date', $expense->date->toDateString()) }}" max="{{ now()->toDateString() }}"
                ...
                <div>
                    ...
                    <textarea id="note" name="note" rows="3"
                              ...
                              class="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm text-charcoal placeholder-charcoal/40 focus:ring-2 focus:ring-latte focus:border-latte transition-colors duration-200 resize-none {{ $errors->has('note') ? 'border-red-400 focus:ring-red-300 focus:border-red-400' : '' }}">{{ old('note', $expense->note) }}</textarea>
                    @error('note')
                        <p class="mt-1.5 text-xs text-red-500 font-medium">{{ $message }}</p>
                    @enderror
                </div>
